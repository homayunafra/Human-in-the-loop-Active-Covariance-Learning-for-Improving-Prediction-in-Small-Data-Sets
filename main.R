...
mseMatrixTe = matrix(0,num_iterations,num_methods)
...
for(numRun in 1:50){
  ...
  # c_star              = syn_data$c_star
  # model_data          = list(x_tr = x_tr, x_te = x_te, y_tr = y_tr, y_te = y_te, beta_star = beta_star, c_star = c_star, mse_gt = mse_gt, fd = feature_descriptors)
  ...
  Utility = compute_utilities(fitted_model,feedback_list,feature_descriptors,N,M,d,y_tr,x_tr,a_sigma,b_sigma,a_tau,b_tau,muStar,vStar,a_sigma_star,b_sigma_star,c(),c(),q,threshold_0)
  nonseq_utility = Utility
  seq_utility = Utility

  random_feedback_list = feedback_list
  random_feedback_table = feedback_table

  nonseq_feedback_list = feedback_list
  nonseq_feedback_table = feedback_table

  seq_feedback_list = feedback_list
  seq_feedback_table = feedback_table
  ...
  random_feedback_total = c()
  nonseq_feedback_total = c()
  seq_feedback_total = c()

  for(iteration in 1:num_iterations){
    if(iteration == 1){
      mseMatrixTe[iteration,1:3] = mse(x_te%*%muStar, y_te)
    }else{
      ...
      mseMatrixTe[iteration,1] = mse(x_te%*%random_beta, y_te)
      ...
      mseMatrixTe[iteration,2] = mse(x_te%*%nonseq_beta, y_te)
      
      ##################### Infogain - Sequential ######################

      if(iteration > 2){
        q = 1 / (1 + exp(seq_Wf_All%*%seq_gamma - seq_threshold))
        seq_utility = compute_utilities(fitted_model, seq_feedback_list, feature_descriptors, N, M, d, y_tr, x_tr, a_sigma, b_sigma, a_tau = a_tau, b_tau = b_tau, seq_beta, seq_vStar, seq_a_sigma_star, seq_b_sigma_star, seq_Wf, seq_feedback_total,q,threshold_0)
      }
      ...
      mseMatrixTe[iteration,3] = mse(x_te%*%seq_beta, y_te)
      mseMatrixTr[iteration,3] = mse(x_tr%*%seq_beta,y_tr)
    }
  }
}
